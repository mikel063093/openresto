    ...
    IAuthCookieService cookies) : ControllerBase
{
    ...
    public async Task<IActionResult> Login([FromBody] LoginRequest req)
    {
        ...
        if (jwt == null)
            return Unauthorized(new { message = ApiLocalization.Localize(HttpContext, "Invalid email or password.") });
        _cookies.SetCookie(Response, jwt);
        return Ok(new { message = ApiLocalization.Localize(HttpContext, "Login successful.") });
    }
    ...
    public IActionResult Logout()
    {
        ...
        return Ok(new { message = ApiLocalization.Localize(HttpContext, "Logged out.") });
    }
    ...
    public async Task<IActionResult> ChangePassword([FromBody] ChangePasswordRequest req)
    {
        ...
        if (!ok)
            return Unauthorized(new { message = ApiLocalization.Localize(HttpContext, "Current password is incorrect.") });
        return Ok(new { message = ApiLocalization.Localize(HttpContext, "Password changed successfully.") });
    }
    ...
    public async Task<IActionResult> ChangeEmail([FromBody] ChangeEmailRequest req)
    {
        ...
        return Ok(new
        {
            message = ApiLocalization.Localize(HttpContext, "Email changed successfully."),
            email = req.NewEmail!.Trim().ToLowerInvariant()
        });
    }
    ...
    public async Task<IActionResult> SetupPvq([FromBody] SetupPvqRequest req)
    {
        if (string.IsNullOrWhiteSpace(req.Question) || string.IsNullOrWhiteSpace(req.Answer))
            return BadRequest(new { message = ApiLocalization.Localize(HttpContext, "Question and answer are required.") });

        await _securityQuestions.SetupAsync(req.Question, req.Answer);
        return Ok(new { message = ApiLocalization.Localize(HttpContext, "Security question configured.") });
    }
    ...
    public async Task<IActionResult> VerifyPvq([FromBody] VerifyPvqRequest req)
    {
        ...
        {
            PvqVerifyStatus.NotConfigured => BadRequest(new { message = ApiLocalization.Localize(HttpContext, "Security question not configured for this account.") }),
            PvqVerifyStatus.WrongAnswer => Unauthorized(new { message = ApiLocalization.Localize(HttpContext, "Incorrect answer.") }),
    ...
    public async Task<IActionResult> ResetPassword([FromBody] ResetPasswordRequest req)
    {
        ...
        if (!ok)
            return BadRequest(new { message = ApiLocalization.Localize(HttpContext, "Invalid or expired reset token.") });
        return Ok(new { message = ApiLocalization.Localize(HttpContext, "Password reset successfully.") });
    }
}
