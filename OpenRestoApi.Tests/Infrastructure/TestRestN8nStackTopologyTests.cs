...
public sealed class TestRestN8nStackTopologyTests
{
    ...
    public void TestRestTraefik_ExposesOnlyN8nWebhookPaths_AndKeepsBotPrivate()
    {
        ...
        Assert.Contains("middlewares:", text, StringComparison.Ordinal);
        Assert.Contains("- n8n-test-webhook-rate-limit", text, StringComparison.Ordinal);
        Assert.Contains("- n8n-test-webhook-inflight", text, StringComparison.Ordinal);
        Assert.Contains("- n8n-test-webhook-buffer", text, StringComparison.Ordinal);
        Assert.Contains("n8n-test-webhook-rate-limit:", text, StringComparison.Ordinal);
        Assert.Contains("average: 30", text, StringComparison.Ordinal);
        Assert.Contains("burst: 60", text, StringComparison.Ordinal);
        Assert.Contains("period: 1m", text, StringComparison.Ordinal);
        Assert.Contains("n8n-test-webhook-inflight:", text, StringComparison.Ordinal);
        Assert.Contains("amount: 20", text, StringComparison.Ordinal);
        Assert.Contains("n8n-test-webhook-buffer:", text, StringComparison.Ordinal);
        Assert.Contains("maxRequestBodyBytes: 1048576", text, StringComparison.Ordinal);
        Assert.Contains("memRequestBodyBytes: 262144", text, StringComparison.Ordinal);
