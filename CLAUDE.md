...
- **PostgreSQL is an infrastructure overlay, not a provider switch**: `docker-compose.postgres.yml` defines an internal-only PostgreSQL 16 service (`postgres:5432` on `postgres-internal`, no host port) and sets `DATABASE_PROVIDER=postgres`. Use it only with a separately reviewed PostgreSQL-capable image (Npgsql/provider support and provider-compatible migrations); a connection string by itself does not change an EF provider. PostgreSQL values (`POSTGRES_DB`, `POSTGRES_USER`, `POSTGRES_PASSWORD`) must remain in the protected deployment environment. `CONNECTION_STRING` targets Docker DNS `Host=postgres;Port=5432`, never localhost.
- **Schema migrations vs data migrations**: EF migrations are schema-only, versioned and preferably additive. Prove fresh and upgraded schemas converge, deploy compatibility code before destructive constraints/drops, and take/verify a backup before production startup auto-migration. Business/data rewrites are explicit, idempotent, batched jobs with checkpoints, metrics, validation queries, and a rollback or forward-fix plan—never an opaque large rewrite inside `Up()`.
- **SQLite → PostgreSQL cutover**: rehearse a repeatable import from a sanitized SQLite snapshot into an isolated PostgreSQL environment; validate counts, foreign keys, UTC values, admin access and booking flows. During a maintenance write freeze, take a final SQLite backup, import, validate, then start only the PostgreSQL-capable release. Never keep both stores writable. Before PostgreSQL accepts writes, rollback means restarting the known-good SQLite release from that final backup; after writes begin, use a rehearsed reverse path or repair forward. Keep SQLite read-only backups through the validation window.
- **PostgreSQL recovery operations**: `scripts/postgres-backup.sh` creates a custom `pg_dump` archive, `pg_restore --list` manifest and SHA-256 checksum; it can optionally use environment-only restic configuration plus a failure alert hook. `scripts/postgres-restore.sh` requires checksum verification, a stopped backend, `--apply`, and an exact `--confirm-database` guard. Run `scripts/postgres-restore-drill.sh` quarterly against a disposable isolated PostgreSQL 16 container. See `docs/backup-restore.md` for the full runbook.
