...
- For `whatsapp-reservations-test`, Phase 8 has started with passing local regression gates, including fail-closed webhook/bootstrap changes and an enforced test-stack network/image hardening pass: `n8n-test` no longer shares an egress network with the backend, and n8n requires an immutable image digest rather than a mutable tag. As of the 19:45 UTC readiness gate on July 30, the focused boundary/security suite passed (12/12), compose validated with non-secret sentinels and failed closed for five independently removed required inputs, and targeted n8n/runbook Gitleaks scans found no leaks. The test-only dynamic Traefik file on the active Docker host matches the repository and includes only the intended webhook route, but no `test-rest` stack is deployed. Phase 8 cannot close until the external prerequisites and a test deployment are available. It is blocked on Meta/WABA, test number, test DNS/TLS (the n8n host does not resolve), all 21 externally injected deployment inputs (including the immutable n8n image digest), webhook registration, applicable template approval, and a healthy `test-rest` edge (the public probe returned `502`).
